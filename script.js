  ...
  themeToggle.addEventListener('click', () => {
    ...
    themeToggle.innerHTML = isDark ? '<i class="fas fa-moon fa-lg"></i>' : '<i class="fas fa-sun fa-lg"></i>';
  });
  ...
  function applyFilters() {
    const activeFilters = Array.from(filters).filter(i => i.checked).map(i => i.value);
    let count = 0;
    products.forEach(p => {
      const brand = p.getAttribute('data-marca');
      if (activeFilters.length === 0 || activeFilters.includes(brand)) {
